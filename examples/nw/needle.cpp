        ...
        int max_rows, int max_cols, int penalty)
{
    ...
    total_in += sizeof(int) + sizeof(int) + sizeof(int)*transfer_size + sizeof(int)*transfer_size;
    total_out += sizeof(int)*transfer_size;
#pragma omp target data map(max_cols, penalty, referrence[0:transfer_size]) map(input_itemsets[0:transfer_size])
#endif
    {

#ifdef OMP_OFFLOAD_NOREUSE
    total_in += sizeof(int) + sizeof(int) + sizeof(int)*transfer_size + sizeof(int)*transfer_size;
    total_out += sizeof(int) + sizeof(int) + sizeof(int)*transfer_size + sizeof(int)*transfer_size;
#pragma omp target data map(max_cols, penalty, referrence[0:transfer_size], input_itemsets[0:transfer_size])
#endif
#pragma omp target 
#endif
